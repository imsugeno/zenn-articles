---
title: "AWSクロスアカウントでAPI GatewayをECSに繋ぐとヘルスチェックは通るのに疎通せずハマりまくった"
emoji: "🪤"
...
出典: AWS [Update the security groups for your Network Load Balancer](https://docs.aws.amazon.com/elasticloadbalancing/latest/network/load-balancer-update-security-groups.html)
:::
...
## 罠3: 証明書エラーで TLS が通らない／VPC Endpoint の名前が引けない
...
### 壁2: VPC Endpoint の自動 DNS 名は VPC の内側からしか引けない

プライベート DNS を無効（`private_dns_enabled = false`。VPC Endpoint の自動 DNS 名を VPC 内の DNS に登録しない設定）にすると、`vpce-….vpce-svc-….vpce.amazonaws.com` という自動名は、その VPC の内側からしか名前解決できません。外（公開 DNS）から使うには、別名で指し直す必要があります。
...
- 内部通信パスは WAF で明示 Allow ＋独立した認証で多層防御
