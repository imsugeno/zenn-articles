...
バックエンドの運用では、500系のエラーが発生した際にエラーログの割合や何らかのメトリクスをもとにSlackなどのチャットツールへアラートを通知する監視ワークフローを組むパターンが多いのではないでしょうか？
...
Slack通知にはSlack側でWebhook URLを発行し、Playbook内のプロンプトにハードコードしていますが、DevinのSecrets機能を使えば確実定数として取り込めると思います。
